...
unsigned int nStakeMinAge = 45 * 24 * nOneHour; // 45 days as zero time weight
...
unsigned int nStakeTargetSpacingNew = 5 * 60; // 3-minute stakes spacing
int64_t nTargetSpacing = 3 * 60;  // Same as the above
int64_t nTargetSpacingNew = 5 * 60;  // Same as the above
...
const string strMessageMagic = "ForkCoin Signed Message:\n";
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("ForkCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ForkCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // ForkCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ForkCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "FORK COIN";
        CTransaction txNew;
        txNew.nTime = 1509550827;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1509550827;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1705354; // !fTestNet ? 1575379 : 46534;
