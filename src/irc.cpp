...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ForkCoinTEST2\r");
            Send(hSocket, "WHO #ForkCoinTEST2\r");
        } else {
            // randomly join #ForkCoin00-#ForkCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ForkCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ForkCoin%02d\r", channel_number).c_str());
        }
