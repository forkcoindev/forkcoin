ForkCoin-qt: Qt4 GUI for ForkCoin
===============================
...
Alternatively, install Qt Creator and open the `ForkCoin-qt.pro` file.

An executable named `ForkCoin-qt` will be built.
...
A warning for people using the *static binary* version of ForkCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of ForkCoin is linked against libdb5.3.
...
installed causes ForkCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
